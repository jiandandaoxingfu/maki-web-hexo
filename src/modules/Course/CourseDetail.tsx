...
export function CourseDetail({ course }: { course: CourseModel }) {
  const created_at = course.created_at.match(/\d{4}.\d{2}.\d{2}/)?.[0];
  return (
    <>
      <Content>
        <Row>
        ...
        <Row>
          <Space size={50} align={ 'start' }>
            <Col className='course-detail-header'>
              <Image preview={false} width={200} height={270} src={course.cover} />
            </Col>
...
export function CourseDetailPage({ id }: { id: string }) {
  ...
  return (
      <Skeleton avatar active title={false} paragraph={{ rows: 15 }} loading={loading}>
